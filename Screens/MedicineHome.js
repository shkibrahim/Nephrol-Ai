...
const MedicineHome = ({ navigation}) => {
  ...
  const fetchReminders = async() => {
    ...
    axios.get(`http://172.20.10.4:8080/patient/allReminder/${patientEmail}`)
  ...
  const updateMedicine = async (reminderID) => {
    ...
    try {
      const response = await axios.patch(`http://172.20.10.4:8080/patient/updateReminder/${reminderID}`, {
...
const addMedicine=()=>{
  ...
  const deleteReminder=async(reminderID)=>{
   try{
    ...
    const res=await axios.delete(`http://172.20.10.4:8080/patient/deleteReminder/${reminderID}`)
